...
static int32_t chip8_worker(void* context) {
    ...
    while(1) {
        ...
        if(chip8->st.worker_state == WorkerStateLoadingRom) {
            ...
            if(!t_chip8_load_game(chip8->st.t_chip8_state, rom_data, rom_len)) {
                FURI_LOG_E(WORKER_TAG, "ROM is invalid or too large: %d bytes", rom_len);
                chip8->st.worker_state = WorkerStateRomLoadError;
                break;
            }
        ...
        if(chip8->st.worker_state == WorkerStateRomLoaded) {
            ...
            if(t_chip8_get_fault(chip8->st.t_chip8_state) != Chip8FaultNone) {
                FURI_LOG_E(
                    WORKER_TAG,
                    "VM stopped safely, fault=%u",
                    (unsigned)t_chip8_get_fault(chip8->st.t_chip8_state));
                chip8->st.worker_state = WorkerStateRomLoadError;
                continue;
            }
            FURI_LOG_I(
...
Chip8Emulator* chip8_make_emulator(const FuriString* file_path) {
    furi_assert(file_path);
    FURI_LOG_I("CHIP8", "make emulator, file_path=%s", furi_string_get_cstr(file_path));

    Chip8Emulator* chip8 = malloc(sizeof(Chip8Emulator));
    chip8->file_path = furi_string_alloc_set(file_path);
...
void chip8_toggle(Chip8Emulator* chip8) {
    furi_assert(chip8);
    chip8->st.t_chip8_state->go_render = false;
}
...
uint16_t read_rom_data(File* file, uint8_t* data) {
    ...
    while(1) {
        const size_t remaining = 4096U - file_pointer;
        if(remaining == 0) break;
        uint16_t bytes_were_read =
            storage_file_read(file, buff, MIN((size_t)buffer_size, remaining));
