...
void chip8_set_file_name(Chip8View* chip8, const FuriString* name) {
    furi_assert(name);
    UNUSED(chip8);
}
...
void chip8_set_backup_screen(Chip8View* chip8, uint8_t** screen) {
    furi_assert(screen);
    with_view_model(chip8->view, Chip8Model * model, { model->backup_screen = screen; }, false);
}

void chip8_set_state(Chip8View* chip8, Chip8State* st) {
    furi_assert(st);
    with_view_model(
        chip8->view, Chip8Model * model, { memcpy(&model->state, st, sizeof(Chip8State)); }, true);
}
